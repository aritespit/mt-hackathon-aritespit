...
def read_and_save():
    ...
    for file in os.listdir(folder_path):
        if file.endswith('.csv'):
            ...
            df = df[df['Handle'].str.lower() == f'{file.split(".")[0].lower()}']
            datasets.append(df)

    combined_dataset = pd.concat(datasets)
    print(combined_dataset['Handle'].unique())
    print(len(combined_dataset['Handle'].unique()))

    columns_to_keep = ['index', 'Name', 'Handle', 'Timestamp', 'Content', 'Photo Link', 'Tweet ID']
    ...
    combined_dataset.rename(columns={'Photo Link': 'photo_link', 'Tweet ID':'tweet_id'}, inplace=True)
    combined_dataset.dropna(subset=['Content'], inplace=True)
